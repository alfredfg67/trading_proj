...
def calculate_all_metrics(df):
    ...
    metrics = {
        ...
        "win_loss_ratio": abs(df[df["profit"] > 0]["profit"].mean() / df[df["profit"] < 0]["profit"].mean()) if (df["profit"] < 0).any() and (df["profit"] > 0).any() else 0,
...
def compare_periods(df, baseline_metrics):
    """Calculate delta vs previous period for KPI cards"""
